...
export const SAVED_KEY   = "replyjet_saved";
...
/** Toggle a saved reply — returns new saved state (true = bookmarked) */
export function toggleSaved({ id, message, tone, language, reply, intent, mode }) {
  try {
    ...
    if (exists !== -1) {
      ...
      const entry = { id, ts: Date.now(), message, tone, language, reply, intent, mode };
      next = [entry, ...prev].slice(0, SAVED_MAX);
    }
    localStorage.setItem(SAVED_KEY, JSON.stringify(next));
    return exists === -1; // true = just saved
  } catch { return false; }
}

/** Check if a reply id is saved */
